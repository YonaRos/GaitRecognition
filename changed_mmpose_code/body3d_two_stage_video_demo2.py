...
def main():
    ...
    for i, pose_det_results in enumerate(
        ...
        for idx, res in enumerate(pose_lift_results):
            ...
            print(keypoints_3d) #we need to print it too if we use the main/main2 code!!
            ...
            print(res) #the print that we use!!!!!!!!
            pose_lift_results_vis.append(res)
        #add_to_file.write("\n\n\n\n\n")
        #add_to_file.write("frame number: " + str(counter) )
        #!@@!@!@!@!@!@!@!@!@@!@!@!@!@!@!@!@@@!@!@!@!@!@!@!@!@!@!@!@!@!@if you want the full run, you remove the two next lines - put in comments
        #add_to_file.close()  #force close
        #return
        counter += 1
        # print("ofir_solman")
        #add_to_file.write(str(pose_lift_results))
        for frame in pose_lift_results:
            #print("frame number: " + str(counter))
            tracking_id = str(frame["track_id"])
            #print("track number: " + tracking_id)
            if tracking_id in data_all.keys():
              data_all[tracking_id][str(counter-1)] = frame['keypoints_3d']
              #print(data_all)
            else:
              data_all[tracking_id] = {}
              data_all[tracking_id][str(counter-1)] = frame['keypoints_3d']
            #print(frame["keypoints"][1])
            """if(counter == 2):
                print(frame["keypoints"][1])
                print(temp)
                print( [ frame["keypoints"][1][0] - temp[0] , frame["keypoints"][1][1] - temp[1], frame["keypoints"][1][2] - temp[2] ] )
            else:
                temp = frame["keypoints"][1]
            #print(data_dict)
            if tracking_id in data_dict:
                data_dict[tracking_id]["keypoints"].append(frame["keypoints"])
                data_dict[tracking_id]["keypoints_3d"].append(frame["keypoints_3d"])
                data_dict[tracking_id]["frame_num"] += 1
                #print(tracking_id + "and " + str(data_dict[tracking_id]["frame_num"]) +"\n\n")
                
                
                if data_dict[tracking_id]["frame_num"] > 1:
                    distance_list[str(counter)] = math.sqrt(math.pow(frame["keypoints"][-1][0] - frame["keypoints"][-2][0],2) + math.pow(frame["keypoints"][-1][1] - frame["keypoints"][-2][1],2) + math.pow(frame["keypoints"][-1][2] - frame["keypoints"][-2][2],2) )
                    data_dict_diff1[tracking_id]["keypoints"].append(frame["keypoints"][-1] - frame["keypoints"][-2])
                    data_dict_diff1[tracking_id]["keypoints_3d"].append(frame["keypoints"][-1] - frame["keypoints"][-2])
                  
                if data_dict[tracking_id]["frame_num"] % 3 == 0:
                    data_dict_diff3[tracking_id]["keypoints"].append(frame["keypoints"][-1] - frame["keypoints"][-3])
                    data_dict_diff3[tracking_id]["keypoints_3d"].append(frame["keypoints"][-1] - frame["keypoints"][-3])
                    
                if data_dict[tracking_id]["frame_num"] % 5 == 0:
                    data_dict_diff5[tracking_id]["keypoints"].append(frame["keypoints"][-1] - frame["keypoints"][-5])
                    data_dict_diff5[tracking_id]["keypoints_3d"].append(frame["keypoints"][-1] - frame["keypoints"][-5])
                    
                if data_dict[tracking_id]["frame_num"] % 10 == 0:
                    data_dict_diff10[tracking_id]["keypoints"].append(frame["keypoints"][-1] - frame["keypoints"][-10])
                    data_dict_diff10[tracking_id]["keypoints_3d"].append(frame["keypoints"][-1] - frame["keypoints"][-10])
                
                
            else:
                data_dict[tracking_id] = {"keypoints": [] , "keypoints_3d": [] , "frame_num": 1}
                data_dict[tracking_id]["keypoints"].append(frame["keypoints"])
                data_dict[tracking_id]["keypoints_3d"].append(frame["keypoints_3d"])
                
                data_dict_diff1[tracking_id] = {"keypoints": [] , "keypoints_3d": []}
                #data_dict_diff1[tracking_id]["keypoints"].append(frame["keypoints"])
                #data_dict_diff1[tracking_id]["keypoints_3d"].append(frame["keypoints_3d"])
                
                data_dict_diff3[tracking_id] = {"keypoints": [] , "keypoints_3d": []}
                #data_dict_diff3[tracking_id]["keypoints"].append(frame["keypoints"])
                #data_dict_diff3[tracking_id]["keypoints_3d"].append(frame["keypoints_3d"])
                
                
                data_dict_diff5[tracking_id] = {"keypoints": [] , "keypoints_3d": []}
                #data_dict_diff5[tracking_id]["keypoints"].append(frame["keypoints"])
                #data_dict_diff5[tracking_id]["keypoints_3d"].append(frame["keypoints_3d"])
                
                
                
                data_dict_diff10[tracking_id] = {"keypoints": [] , "keypoints_3d": []}
                #data_dict_diff10[tracking_id]["keypoints"].append(frame["keypoints"])
                #data_dict_diff10[tracking_id]["keypoints_3d"].append(frame["keypoints_3d"])"""
        ...
        img_vis = vis_3d_pose_result(
            ...
            show=args.show)
        #print("do we print?: "+str(save_out_video))
        if save_out_video:
            if writer is None:
    ...
    f = open("/data/students/ofiryonatan_data/last_data/new_tensors/" + osp.basename(args.video_path), "a")
    f.write(str(data_all))
    f.close()
    add_to_file.write(str(data_dict_diff10) )
    #print(data_dict)
    #print(data_dict_diff1)
    """print(len(data_dict["0"]["keypoints"]))
    print(len(data_dict_diff1["0"]["keypoints"]))
    print(len(data_dict_diff3["0"]["keypoints"]))
    print(len(data_dict_diff5["0"]["keypoints"]))
    print(len(data_dict_diff10["0"]["keypoints"]))
